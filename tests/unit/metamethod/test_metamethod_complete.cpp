...
static Table* createTestTable(ScopedGCRoots& roots) {
    return roots.create<Table>();
}

static Function* createTestFunction(ScopedGCRoots& roots, CFunction function) {
    return roots.create<Function>(function);
}

static Table* createVector(ScopedGCRoots& roots, f64 x, f64 y) {
...
static i32 vector_sub(LuaState* L) {
    ...
    Table* result = createVector(roots, x1 - x2, y1 - y2);
    L->pushTable(result);
    return 1;
}
...
static i32 vector_mul(LuaState* L) {
    ...
    ScopedGCRoots roots(L);
    Table* result = createVector(roots, x * scalar, y * scalar);
    L->pushTable(result);
    return 1;
}
...
void testArithmeticMetamethods(TestSuite& suite) {
    ...
    // 创建元表
    Table* mt = createTestTable(roots);
    mt->set(Value(pool.intern("__sub")), Value(createTestFunction(roots, vector_sub)));
    mt->set(Value(pool.intern("__mul")), Value(createTestFunction(roots, vector_mul)));
    mt->set(Value(pool.intern("__div")), Value(createTestFunction(roots, vector_div)));

    // 测试 __sub
    Table* v1 = createVector(roots, 5.0, 7.0);
    Table* v2 = createVector(roots, 2.0, 3.0);
    v1->setMetatable(mt);
    v2->setMetatable(mt);
    ...
    // 测试 __mul
    Table* v3 = createVector(roots, 3.0, 4.0);
    v3->setMetatable(mt);
    Value mulResult;
    ...
    // 测试 __div
    Table* v4 = createVector(roots, 10.0, 20.0);
    v4->setMetatable(mt);
    Value divResult;
...
void testComparisonMetamethods(TestSuite& suite) {
    ...
    // 创建元表
    Table* mt = createTestTable(roots);
    mt->set(Value(pool.intern("__eq")), Value(createTestFunction(roots, vector_eq)));
    mt->set(Value(pool.intern("__lt")), Value(createTestFunction(roots, vector_lt)));
    mt->set(Value(pool.intern("__le")), Value(createTestFunction(roots, vector_le)));

    // 创建测试向量
    Table* v1 = createVector(roots, 3.0, 4.0); // 长度 5
    Table* v2 = createVector(roots, 6.0, 8.0); // 长度 10
    Table* v3 = createVector(roots, 3.0, 4.0); // 长度 5，与v1相等
    v1->setMetatable(mt);
    v2->setMetatable(mt);
    v3->setMetatable(mt);
...
void testOtherMetamethods(TestSuite& suite) {
    ...
    // 测试 __len
    Function* lenFunc = createTestFunction(roots, vector_len);

    Table* v1 = createVector(roots, 3.0, 4.0); // 长度应该是5
...
void testModAndPow(TestSuite& suite) {
    ...
    // 测试 __mod
    Table* mt_mod = createTestTable(roots);
...
void testMetamethodEdgeCases(TestSuite& suite) {
    ...
    // 测试：没有元方法的表
    Table* noMT = createTestTable(roots);
    ...
    // 测试：只有一个操作数有元方法
    Table* mt_add = createTestTable(roots);
    ...
    mt_add->set(Value(addName), Value(createTestFunction(roots, simple_add)));

    Table* hasMetamethod = createTestTable(roots);
    hasMetamethod->setMetatable(mt_add);

    Table* noMetamethod = createTestTable(roots);
...
void registerMetamethodCompleteTests() {
    ...
    registry.registerTest("Complete Metamethods", "Arithmetic metamethods (__sub, __mul, __div)",
                          testArithmeticMetamethods);
    registry.registerTest("Complete Metamethods", "Comparison metamethods (__eq, __lt, __le)",
                          testComparisonMetamethods);
