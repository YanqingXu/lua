...
* 本文件所在提交新增 context-owned `ExecutionPolicy` 第一阶段：主 State 与 coroutine 共享指令预算、`steady_clock` deadline 和单向 atomic cancellation；预算跨 yield/resume 与 C→Lua→C 重入不重置，三类超限均通过预分配固定错误对象进入 protected API。默认关闭热路径的 VM 指令吞吐已纳入 base-vs-head 回归策略。
* [`8e2d884`](https://github.com/YanqingXu/lua/commit/8e2d88412691c03d060685841935f44e6817ec77) 闭环 `finalizerBudgetPerDrain`：默认无限以保持 Lua 5.1 行为；有限值按每次完整 GC、增量 finalize 阶段或 `lua_close` drain 重新补充。普通 GC 未消费的 userdata 保持 rooted 并留待后续周期，关闭时达到上限后停止进入用户回调但仍无异常释放全部 allocator-backed 存储。
* 本文件所在提交闭环固定 owner-thread 合同：`GlobalState` 构造线程不可转移，`EngineContext`/`RuntimeServices`、VM 和公开 C API 在接触可变状态前拒绝 foreign thread；protected/`noexcept` API 保持栈不变并返回失败值，foreign-thread `lua_close` 无操作且必须由 owner 完成销毁。唯一跨线程能力仍是 owner 预先取得的 atomic cancellation handle。
* 本文件所在提交新增 context-owned `SandboxPolicy`：默认 unrestricted 保持 Lua 5.1 行为，game-server profile 只开放 base/math/string/table/coroutine 与 preload-only package；库 opener 在发布前校验，文件系统、进程和原生模块操作在每次调用重新校验，已捕获函数无法绕过后续收紧。四类拒绝使用预分配 fixed 错误对象，原生模块拒绝发生在 OS loader 前。可信宿主 C API、preload/native callback 与已加载代码的边界已单独文档化。
* 本文件所在提交闭环长期原生 C/C++ callback 的协作式取消合同：新增可抛项目扩展 `lua_checkexecution`，在有界工作切片之间轮询 context cancellation 与 monotonic deadline，不消费 Lua instruction budget。foreign thread 仍只能调用预先取得的 atomic cancellation handle；不轮询的无界循环和阻塞调用不会被异步抢占，必须使用宿主 timeout/中断机制。
* 本文件所在提交闭环 allocator hard-limit 的 collector-owned worklist 切片：root、gray、weak-table、pending-finalizer、finalizer drain copy 和 cross-collector mark queue 均由 `lua_Alloc` 支撑；完整收集逐点 fail-on-N 和零余量 hard-limit 测试证明 OOM 后根图与 fixed error 稳定、终结器不丢失也不重复、解除限制可重试且关闭后 live bytes 归零。
* 本文件所在提交闭环 allocator hard-limit 的 Table hash 切片：公开 `lua_rawset` 与真实 VM `SETTABLE` 在 callback 实际观察到的每个分配 offset 都返回 fixed `LUA_ERRMEM`、不发布部分 entry、解除限制可重试；零余量 hard limit 全程满足 `liveBytes <= limit`，关闭后 old-size/ownership ledger 清零。
* 本文件所在提交闭环 allocator hard-limit 的分片 reader 聚合缓冲切片：`lua_load` 使用 `LuaStdAllocator<char>` 支撑的 source buffer，EOF 前每个实际增长 offset 都返回 fixed `LUA_ERRMEM`、保留调用者栈前缀、解除限制可重试，且零余量 hard limit 不被突破。这不包括仍使用默认 allocator 的 parser/AST/codegen 临时对象。
* 本文件所在提交闭环 services-backed Lexer `InputCursor` 重放缓存切片：游标以值快照保存 `lua_Alloc` callback/userdata 并用 `LuaVector<i32>` 缓存源码，避免依赖已销毁的 allocator 对象；`loadbuffer` 的动态 fail-on-N 扫描覆盖每个新增游标增长点，零余量 hard limit 证明 fixed `LUA_ERRMEM`、不突破上限、解除限制可重试且关闭归零。Lexer 的 source/lexeme/token、Parser/AST/CodeGen 其他临时对象仍未纳入。
* 本文件所在提交闭环 services-backed Lexer 整源副本与词素累积缓冲切片：Lexer 以值快照保存 callback/userdata，`sourceStorage_` 与 `lexemeBuffer_` 使用 allocator-aware `LuaOwnedString` 并先于快照析构；长标识符 `loadbuffer` 矩阵在保留 Table/for 路径的同时新增 7 个实际 callback offset，逐点证明 fixed `LUA_ERRMEM`、可重试和关闭归零。Token/value/error 字符串以及 Parser/AST/CodeGen 其他临时对象仍未纳入；同时移除了 Token 字符串复制入口错误的 `noexcept`，避免默认 allocator OOM 直接终止进程。
* 本文件所在提交闭环 services-backed Token 字符串切片：`LuaSnapshotStdAllocator` 按值持有 callback/userdata，Lexer source/lexeme 与 Token lexeme/decoded-value/error 使用 23 字节内联、外部容量精确记账的 `LuaBasicString` / `LuaOwnedString`，复制和移动后、离开原 allocator 对象作用域后仍能正确释放，也避开 libc++ `basic_string` 的 25→24 `osize` 偏差；无自定义 callback 的独立 Lexer 继续以 `Str` 暴露字符串 variant，保持既有 `get_if<Str>` 行为。长标识符矩阵覆盖全部实际 callback offset；独立生命周期测试证明快照析构归零。Token 借用 view 进入 Parser/AST 后的默认字符串仍未纳入。
* 本文件所在提交闭环 services-backed Parser 函数语法作用域切片：Parser 按值快照 callback/userdata，`functionScopes_` 及每层 locals/upvalues 使用 `LuaOwnedVector<LuaOwnedString>`，长局部名和捕获名的外部载荷也按精确容量走 `lua_Alloc`。嵌套闭包 `loadbuffer` 矩阵动态扫描 137 个实际 callback offset，并以 8 次同名精确载荷分配的下限防止 Parser 名称静默退回默认 allocator；逐点证明 fixed `LUA_ERRMEM`、解除限制后可编译并返回正确结果、零余量不超限且关闭归零。AST 内部字符串/容器、诊断与 codegen 临时对象仍未纳入。
* 本文件所在提交闭环 services-backed AST 节点对象切片：`ExprPtr` / `StmtPtr` 使用按值快照 callback/userdata 的 `LuaOwnedPtr`，对象构造失败先释放未发布块，自定义 deleter 以相同精确 `sizeof(Expr)` / `sizeof(Stmt)` 释放，因此 AST 可安全越过 Parser、`RuntimeServices` 与 `EngineContext` 的对象生命周期。嵌套闭包 `loadbuffer` 矩阵扩展到 167 个实际 callback offset，并以跨 Debug/Release 稳定的至少 21 次 Expr、9 次 Stmt 精确尺寸分配防止节点静默退回默认 allocator；逐点 OOM、零余量、解除限制重试与关闭归零继续通过，独立生命周期测试也证明最后一个 AST 节点销毁后 callback live bytes 归零。AST 内部 `Str` / `Vec` 载荷、Parser 诊断与 codegen 临时对象仍未纳入。
* 本文件所在提交闭环 allocator hard-limit 的 State stack/CallInfo 事务证据：`lua_checkstack` 的 fail-on-next 与零余量 hard limit 均保持逻辑栈、allocator live bytes 和旧块不变；protected CallInfo 扩容失败统一为 fixed `LUA_ERRMEM` 并恢复 base call depth，解除限制后两类容量均可增长，后续调用成功且关闭归零。现有 `LuaVector` 实现已满足强保证，无需运行时改动。
* 本文件所在提交闭环 VM `__call` 实参暂存切片：元方法插入 callable self 前使用 State callback 支撑的 `LuaVector<Value>` 保存宽实参；动态矩阵覆盖实参缓冲与必要的 State stack 扩容两个实际 callback offset，逐点证明 fixed `LUA_ERRMEM`、单一错误对象、解除限制可重试且 33 个参数完整、关闭归零；零余量 hard limit 不被突破。
* 本文件所在提交闭环 VM `OP_CONCAT` 临时缓冲切片及跨标准库字符串记账：字符串操作数改为借用 `StrView`，数字格式化使用栈上固定数组，最终拼接结果使用直接维护精确 callback 容量的 `LuaReallocVector<char>` 后交给 allocator-backed StringPool；`GCString` 的不可变外部载荷也改为精确 `length + 1` callback 块，避开 libc++ `basic_string` 在 24 字节边界出现的 25→24 `osize` 偏差。长操作数矩阵覆盖全部实际 callback offset，逐点证明 fixed `LUA_ERRMEM`、重试内容完整、零余量不超限和关闭归零。
* 本文件所在提交闭环标准库 `table.concat` 临时缓冲切片：分隔符和字符串元素直接借用 `StrView`，数字元素复用栈上格式化 helper，最终结果使用 State callback 支撑的 `LuaVector<char>`；长字符串、数字与分隔符矩阵覆盖全部实际 callback offset，逐点证明 fixed `LUA_ERRMEM`、精确内容重试、零余量不超限和关闭归零。
* 本文件所在提交闭环标准库 `table.sort` 工作副本和比较器栈快照切片：两类容器均使用 State callback 支撑的 `LuaVector<Value>`，比较器路径保持 `MemoryError` / `bad_alloc` 分类；16 元素 C comparator 矩阵动态覆盖当前标准库实现的全部实际 callback offset，逐点证明 fixed `LUA_ERRMEM`、目标表不部分发布、解除限制后可排序、零余量不超限和关闭归零。
* 当前本地 Debug/Release strict 基线为 **766 tests / 6482 assertions / 0 failures**，C API 为 **58 tests / 2793 assertions / 0 failures**；上一完整在线基线已全绿，本提交在线矩阵待验证。官方函数合同为 123 PASS、0 XFAIL、0 UNSUPPORTED，项目实际公开面为 131 个函数。核心表/比较、类型/线程/GC、完整 auxlib、调试入口、标准库 opener、原生 callback 执行检查与最后 panic/格式化/环境/cpcall/setlevel 批次，均已具备精确签名、静态/共享导出、直接语义测试和官方 Lua 5.1 纯 C 差分证据。
* [`7e95938`](https://github.com/YanqingXu/lua/commit/7e95938979c82b2f5f2ad9f415bb45f4b83c9a7f) 已在 [Actions run 29505951352](https://github.com/YanqingXu/lua/actions/runs/29505951352) 取得同 SHA 的 **10/10 jobs 全绿**：Windows MSBuild Debug/Release、Linux GCC/Clang Debug/Release、ASan/UBSan、clang-format/clang-tidy 与 schema-v2 paired-run benchmark 均成功；六项性能门全部通过。

审计建议中的最新快照稳定化、C API 生命周期、完整公共 C API/ABI 合同、ExecutionPolicy（含长期原生 callback 协作式取消）、finalizer 单轮预算、固定 owner-thread 合同和 sandbox/module policy 已闭环。剩余主线是 [#5](https://github.com/YanqingXu/lua/issues/5) 的 allocator hard limit；required-check 分支保护仍受私有仓库套餐限制并由 [#6](https://github.com/YanqingXu/lua/issues/6) 跟踪。
