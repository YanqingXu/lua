...
* 本文件所在提交闭环 services-backed Lexer 整源副本与词素累积缓冲切片：Lexer 以值快照保存 callback/userdata，`sourceStorage_` 与 `lexemeBuffer_` 使用 `LuaString` 并先于快照析构；长标识符 `loadbuffer` 矩阵在保留 Table/for 路径的同时新增 7 个实际 callback offset，逐点证明 fixed `LUA_ERRMEM`、可重试和关闭归零。Token/value/error 字符串以及 Parser/AST/CodeGen 其他临时对象仍未纳入；同时移除了 Token 字符串复制入口错误的 `noexcept`，避免默认 allocator OOM 直接终止进程。
* 当前本地 Debug/Release strict 基线为 **759 tests / 5236 assertions / 0 failures**，C API 为 **53 tests / 1555 assertions / 0 failures**；本提交在线矩阵待验证。官方函数合同为 123 PASS、0 XFAIL、0 UNSUPPORTED，项目实际公开面为 131 个函数。核心表/比较、类型/线程/GC、完整 auxlib、调试入口、标准库 opener、原生 callback 执行检查与最后 panic/格式化/环境/cpcall/setlevel 批次，均已具备精确签名、静态/共享导出、直接语义测试和官方 Lua 5.1 纯 C 差分证据。
