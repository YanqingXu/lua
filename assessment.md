...
* 本文件所在提交闭环 `finalizerBudgetPerDrain`：默认无限以保持 Lua 5.1 行为；有限值按每次完整 GC、增量 finalize 阶段或 `lua_close` drain 重新补充。普通 GC 未消费的 userdata 保持 rooted 并留待后续周期，关闭时达到上限后停止进入用户回调但仍无异常释放全部 allocator-backed 存储；sandbox/module policy、owner-thread enforcement 和原生回调协作式取消继续由 [#10](https://github.com/YanqingXu/lua/issues/10) 跟踪。
* 当前本地 Debug/Release strict 基线为 **751 tests / 4845 assertions / 0 failures**，C API 为 **49 tests / 1254 assertions / 0 failures**；官方函数合同为 123 PASS、0 XFAIL、0 UNSUPPORTED，项目实际公开面为 130 个函数。核心表/比较、类型/线程/GC、完整 auxlib、调试入口、标准库 opener 与最后 panic/格式化/环境/cpcall/setlevel 批次，均已具备精确签名、静态/共享导出、直接语义测试和官方 Lua 5.1 纯 C 差分证据。
...
审计建议中的最新快照稳定化、C API 生命周期、完整公共 C API/ABI 合同、ExecutionPolicy 首阶段和 finalizer 单轮预算已闭环。剩余主线是 [#10](https://github.com/YanqingXu/lua/issues/10) 中的 sandbox/module policy、owner-thread enforcement 与原生回调协作式取消，以及 [#5](https://github.com/YanqingXu/lua/issues/5) 的 allocator hard limit；required-check 分支保护仍受私有仓库套餐限制并由 [#6](https://github.com/YanqingXu/lua/issues/6) 跟踪。
