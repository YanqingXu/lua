...
* 当前本地 Debug/Release strict 基线为 **765 tests / 5486 assertions / 0 failures**，C API 为 **58 tests / 1801 assertions / 0 failures**；本提交在线矩阵待验证。官方函数合同为 123 PASS、0 XFAIL、0 UNSUPPORTED，项目实际公开面为 131 个函数。核心表/比较、类型/线程/GC、完整 auxlib、调试入口、标准库 opener、原生 callback 执行检查与最后 panic/格式化/环境/cpcall/setlevel 批次，均已具备精确签名、静态/共享导出、直接语义测试和官方 Lua 5.1 纯 C 差分证据。
* [`8e2d884`](https://github.com/YanqingXu/lua/commit/8e2d88412691c03d060685841935f44e6817ec77) 已在 [Actions run 29423730896](https://github.com/YanqingXu/lua/actions/runs/29423730896) 取得同 SHA 的 **10/10 jobs 全绿**：Windows MSBuild Debug/Release、Linux GCC/Clang Debug/Release、ASan/UBSan、clang-format/clang-tidy 与 base-vs-head benchmark 均成功；首次 benchmark 仅因共享 runner 整体变慢触发噪声，failed-job rerun 的 6/6 性能门全部通过。
