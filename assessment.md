...
* 当前本地 Debug/Release strict 基线为 **738 tests / 4657 assertions / 0 failures**，C API 为 **43 tests / 1101 assertions / 0 failures**；官方函数合同为 75 PASS、48 UNSUPPORTED，项目实际公开面为 82 个函数。核心表、遍历、比较和拼接的 9 个入口，以及类型、线程身份和 GC 控制的 6 个入口，均已具备精确签名、静态/共享导出、直接语义测试，并由同一纯 C probe 对官方 Lua 5.1 做差分。
