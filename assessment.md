...
* 本文件所在提交闭环 services-backed Token 字符串切片：`LuaSnapshotStdAllocator` 按值持有 callback/userdata 并在容器复制、移动、交换时传播，Token 的 lexeme/decoded-value/error 使用 `LuaOwnedString`，离开 Lexer 或原 allocator 对象作用域后仍能正确释放；无自定义 callback 的独立 Lexer 继续以 `Str` 暴露字符串 variant，保持既有 `get_if<Str>` 行为。长标识符矩阵再新增 7 个实际 callback offset；独立生命周期测试证明快照析构归零。Token 借用 view 进入 Parser/AST 后的默认字符串仍未纳入。
...
* 当前本地 Debug/Release strict 基线为 **765 tests / 5476 assertions / 0 failures**，C API 为 **58 tests / 1791 assertions / 0 failures**；本提交在线矩阵待验证。官方函数合同为 123 PASS、0 XFAIL、0 UNSUPPORTED，项目实际公开面为 131 个函数。核心表/比较、类型/线程/GC、完整 auxlib、调试入口、标准库 opener、原生 callback 执行检查与最后 panic/格式化/环境/cpcall/setlevel 批次，均已具备精确签名、静态/共享导出、直接语义测试和官方 Lua 5.1 纯 C 差分证据。
