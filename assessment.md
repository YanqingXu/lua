...
* 本文件所在提交闭环 VM `OP_CONCAT` 临时缓冲切片：字符串操作数改为借用 `StrView`，数字格式化使用栈上固定数组，最终拼接结果使用 State callback 支撑的 `LuaString` 后交给 allocator-backed StringPool；长操作数矩阵覆盖 4 个实际 callback offset，逐点证明 fixed `LUA_ERRMEM`、重试内容完整、零余量不超限和关闭归零。
* 本文件所在提交闭环标准库 `table.concat` 临时缓冲切片：分隔符和字符串元素直接借用 `StrView`，数字元素复用栈上格式化 helper，最终结果使用 State callback 支撑的 `LuaString`；长字符串、数字与分隔符矩阵覆盖 5 个实际 callback offset，逐点证明 fixed `LUA_ERRMEM`、精确内容重试、零余量不超限和关闭归零。
* 当前本地 Debug/Release strict 基线为 **764 tests / 5446 assertions / 0 failures**，C API 为 **57 tests / 1761 assertions / 0 failures**；本提交在线矩阵待验证。官方函数合同为 123 PASS、0 XFAIL、0 UNSUPPORTED，项目实际公开面为 131 个函数。核心表/比较、类型/线程/GC、完整 auxlib、调试入口、标准库 opener、原生 callback 执行检查与最后 panic/格式化/环境/cpcall/setlevel 批次，均已具备精确签名、静态/共享导出、直接语义测试和官方 Lua 5.1 纯 C 差分证据。
