...
* 当前本地 Debug/Release strict 基线为 **740 tests / 4700 assertions / 0 failures**，C API 为 **45 tests / 1144 assertions / 0 failures**；官方函数合同为 100 PASS、23 UNSUPPORTED，项目实际公开面为 107 个函数。核心表、遍历、比较和拼接的 9 个入口，类型、线程身份和 GC 控制的 6 个入口，以及 auxlib 注册、参数检查、metatable 和 buffer 的 25 个入口，均已具备精确签名、静态/共享导出、直接语义测试，并由同一纯 C probe 对官方 Lua 5.1 做差分。
