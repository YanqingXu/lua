...
* 当前本地 Debug/Release strict 基线为 **743 tests / 4774 assertions / 0 failures**，C API 为 **48 tests / 1218 assertions / 0 failures**；官方函数合同为 116 PASS、7 UNSUPPORTED，项目实际公开面为 123 个函数。核心表、遍历、比较和拼接的 9 个入口，类型、线程身份和 GC 控制的 6 个入口，auxlib 注册、参数检查、metatable 和 buffer 的 25 个入口，stack/info/local/hook 的 8 个调试入口，以及 8 个 `luaopen_*` 标准库入口，均已具备精确签名、静态/共享导出、直接语义测试，并由同一纯 C probe 对官方 Lua 5.1 做差分。
