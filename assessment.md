...
* 本文件所在提交闭环 VM `OP_CONCAT` 临时缓冲切片：字符串操作数改为借用 `StrView`，数字格式化使用栈上固定数组，最终拼接结果使用 State callback 支撑且跨标准库具有精确 deallocation 计数的 `LuaVector<char>` 后交给 allocator-backed StringPool；长操作数矩阵覆盖全部实际 callback offset，逐点证明 fixed `LUA_ERRMEM`、重试内容完整、零余量不超限和关闭归零。
...
* 当前本地 Debug/Release strict 基线为 **765 tests / 5474 assertions / 0 failures**，C API 为 **58 tests / 1789 assertions / 0 failures**；本提交在线矩阵待验证。官方函数合同为 123 PASS、0 XFAIL、0 UNSUPPORTED，项目实际公开面为 131 个函数。核心表/比较、类型/线程/GC、完整 auxlib、调试入口、标准库 opener、原生 callback 执行检查与最后 panic/格式化/环境/cpcall/setlevel 批次，均已具备精确签名、静态/共享导出、直接语义测试和官方 Lua 5.1 纯 C 差分证据。
