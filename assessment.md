...
* 本文件所在提交闭环固定 owner-thread 合同：`GlobalState` 构造线程不可转移，`EngineContext`/`RuntimeServices`、VM 和公开 C API 在接触可变状态前拒绝 foreign thread；protected/`noexcept` API 保持栈不变并返回失败值，foreign-thread `lua_close` 无操作且必须由 owner 完成销毁。唯一跨线程能力仍是 owner 预先取得的 atomic cancellation handle；sandbox/module policy 和原生回调协作式取消继续由 [#10](https://github.com/YanqingXu/lua/issues/10) 跟踪。
* 当前本地 Debug/Release strict 基线为 **752 tests / 4864 assertions / 0 failures**，C API 为 **49 tests / 1262 assertions / 0 failures**；官方函数合同为 123 PASS、0 XFAIL、0 UNSUPPORTED，项目实际公开面为 130 个函数。核心表/比较、类型/线程/GC、完整 auxlib、调试入口、标准库 opener 与最后 panic/格式化/环境/cpcall/setlevel 批次，均已具备精确签名、静态/共享导出、直接语义测试和官方 Lua 5.1 纯 C 差分证据。
...
审计建议中的最新快照稳定化、C API 生命周期、完整公共 C API/ABI 合同、ExecutionPolicy 首阶段、finalizer 单轮预算和固定 owner-thread 合同已闭环。剩余主线是 [#10](https://github.com/YanqingXu/lua/issues/10) 中的 sandbox/module policy 与原生回调协作式取消，以及 [#5](https://github.com/YanqingXu/lua/issues/5) 的 allocator hard limit；required-check 分支保护仍受私有仓库套餐限制并由 [#6](https://github.com/YanqingXu/lua/issues/6) 跟踪。
