...
* 当前本地 Debug/Release strict 基线为 **765 tests / 6177 assertions / 0 failures**，C API 为 **58 tests / 2492 assertions / 0 failures**；上一完整在线基线已全绿，本提交在线矩阵待验证。官方函数合同为 123 PASS、0 XFAIL、0 UNSUPPORTED，项目实际公开面为 131 个函数。核心表/比较、类型/线程/GC、完整 auxlib、调试入口、标准库 opener、原生 callback 执行检查与最后 panic/格式化/环境/cpcall/setlevel 批次，均已具备精确签名、静态/共享导出、直接语义测试和官方 Lua 5.1 纯 C 差分证据。
* [`599942b`](https://github.com/YanqingXu/lua/commit/599942bfecf292d539d831e0cab8450b2d6660e6) 已在 [Actions run 29495575432](https://github.com/YanqingXu/lua/actions/runs/29495575432) 取得同 SHA 的 **10/10 jobs 全绿**：Windows MSBuild Debug/Release、Linux GCC/Clang Debug/Release、ASan/UBSan、clang-format/clang-tidy 与 schema-v2 paired-run benchmark 均成功；六项性能门全部通过。
