...
* 当前本地 Debug/Release strict 基线为 **759 tests / 5166 assertions / 0 failures**，C API 为 **53 tests / 1485 assertions / 0 failures**；本提交在线矩阵待验证。官方函数合同为 123 PASS、0 XFAIL、0 UNSUPPORTED，项目实际公开面为 131 个函数。核心表/比较、类型/线程/GC、完整 auxlib、调试入口、标准库 opener、原生 callback 执行检查与最后 panic/格式化/环境/cpcall/setlevel 批次，均已具备精确签名、静态/共享导出、直接语义测试和官方 Lua 5.1 纯 C 差分证据。
