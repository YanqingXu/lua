...
applies_to: 2026-05-23 以前的仓库优化完成史（截至 PR-78）
---

# Lua 仓库优化历史（截至 PR-78）

> 本文档是历史完成记录。当前事实、续接检查清单和下一步任务以
> `docs/roadmap/current.md` 为准。
...
- [x] 同步 README、项目状态、字节码生成说明、职责地图和优化路线图；后续 PR-73 已收口 `LibRegistrar` 自注册评估。
...
### PR-73 / 2.6.2：`LibRegistrar` 是否值得落地

**目标：** 评估是否要把标准库 catalog 从显式 `constexpr` 表改成每个库 `.cpp` 自注册，避免为了“少改一行中心表”引入隐式装配风险。

决策：不落地 `LibRegistrar`。保留 `src/lib/lib_catalog.cpp` 中的显式 catalog 作为唯一事实源。
