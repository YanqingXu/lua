...
| Lexer | `Lexer` | `src/compiler/lexer.hpp` | 把源码切成 Token。 |
| Parser | `Parser` | `src/compiler/parser.hpp` | 把 Token 流转换成 AST；现在支持接收 `RuntimeServices` 的构造重载。 |
| Token | `Token` / `TokenType` | `src/compiler/token.hpp` | Parser 消费的词法单元。 |
...
| ValueResult | `ValueResult` | `src/compiler/codegen_types.hpp` | 表达式右值通道，描述常量、寄存器、pending load 或 call 结果。 |
| CondResult | `CondResult` | `src/compiler/codegen_types.hpp` | 条件表达式通道，携带 true/false 跳转列表。 |
| LValueRef | `LValueRef` | `src/compiler/codegen_types.hpp` | 赋值左侧通道，描述 local、upvalue、global 或 table slot。 |
| CallResultInfo | `CallResultInfo` | `src/compiler/codegen_types.hpp` | 调用/vararg 多返回值通道，记录 base register 和指令位置。 |
...
| Registry | `GlobalState::getRegistry()` | `src/vm/global_state.hpp` | C API 风格的全局注册表，目前由 `GlobalState` 持有。 |
