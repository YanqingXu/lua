...
- 质量门禁配置自检通过。
- 文档漂移检查通过。
- 本机有 MSBuild 和 `bin\lua_test.exe` 时，`run_quality_gate.ps1` 会构建 `lua_test.vcxproj`，并运行 514 个注册测试 / 2515 个结果 / 0 失败。
