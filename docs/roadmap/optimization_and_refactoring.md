...
> 约束：保持 518 个注册测试 / 2580 个断言结果 / 0 失败，不破坏 `LuaState` / `VM` public API。
> 最近审计：2026-05-21（深度审计报告，覆盖 Readability / Extensibility / Educational Value 三维度）
> 最近同步：2026-05-23（PR-56：`lua_bytecode --diff` side-by-side 字节码差异，并同步 Bytecode Tool 文档与漂移守卫）
...
| **阶段 4 — 教育价值增强** | 持续推进 | 自解释代码、字节码可视化、执行链路教学文档、REPL 体验、Trace 差异模式 | 否（增量增强） | ~72% |
...
**现状**：518 个注册测试 / 2580 个断言结果 / 0 失败，目录 `tests/unit/`、`tests/lua/`。
...
| `std::expected` 大范围替换异常导致调用链翻新 | 518 测试可能批量红 | 按 3.1 表格逐个函数迁移，每次 1 个函数 + 全量测试 |
...
| PR | 编号 | 任务 | 阶段 | 依赖 / 理由 |
|---|---|---|---|---|
| PR-57 | 4.4.3 | REPL `.ast` 元命令 | 4 | P0；补齐 REPL 教学入口，让 parser / AST 产物能与 bytecode / diff 输出并排观察 |
| PR-58 | 4.4.3 / 2.3 | REPL `.gc` 元命令前置设计 | 4 / 2 | P1；需先明确 GCStrategy 或现有 GC 统计输出边界，避免把 REPL 命令绑死在单例实现上 |
...
| 阶段 4 — 教育价值增强 | ~72% | **~65%** | −7% | REPL 5 项增强中 4 项未做、字节码 CFG 仍未做 |
...
**结论**：阶段 4 仍是差距最大的阶段。REPL 体验（4.4.2-4.4.5）和字节码 CFG（4.2.6）是两个集中的未完成块。walkthrough 三篇、trace-diff、child proto 递归输出和 `--diff` 字节码对比是亮点。
