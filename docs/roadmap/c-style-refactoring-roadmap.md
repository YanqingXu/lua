...
**完成状态（2026-06-01）：** 阶段 0-6 已实施完成。当前 C-style 护栏剩余登记值为 `NULL=0`、裸 `new=2`、裸 `delete=1`、`std::free/free=1`、简单 `#define=51`、`(void*)=0`、`return nullptr=64`。最终验证通过 `tools\run_quality_gate.ps1`、`bin\lua_test.exe` 全量 `640 registered / 3234 results / 0 failures`、`lua_app.vcxproj` Debug|x64 构建和 `git diff --check`。
