---
status: current
verified_against: docs/PROJECT_STATUS.md; docs/DEVELOPMENT_GUIDE.md; docs/walkthroughs/index.md; docs/glossary.md; examples/README.md
...
1. 读 `docs/PROJECT_STATUS.md`，确认当前构建入口、测试数量和已实现的运行时边界。
2. 读 README 的项目概览，不要把旧的规划项当作当前事实。
3. 读 `docs/DEVELOPMENT_GUIDE.md`，按 Windows / MSBuild 路径构建和验证。
...
2. `docs/BYTECODE_GENERATION.md`：理解当前编译管线。
3. `docs/walkthroughs/index.md`：从测试反推实现。
4. `examples/README.md`：运行小脚本，观察解释器行为。
...
- 接着看 `src/vm/lua_state.hpp`、`src/vm/vm.hpp`、`src/runtime/runtime_services.hpp`。
