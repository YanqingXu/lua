...
`CodeGenerator::generate()` 创建当前 Proto，编译 chunk 中的语句，并在末尾补一条空返回，入口见 `src/compiler/codegen/codegen.cpp:54` 和 `src/compiler/codegen/codegen.cpp:80`。
...
`emitCallExpr()` 的几个关键动作在 `src/compiler/codegen/expression_emitter.cpp:818` 开始：

1. `print` 是全局名，`materializeValue()` 会生成 `GETGLOBAL`，见 `src/compiler/codegen/expression_emitter.cpp:398`。
2. 调用需要函数和参数位于连续寄存器。因为 `print` 先落在 `R0`，而调用基址选择为 `R1`，所以生成 `MOVE R1 R0`，对应 `src/compiler/codegen/expression_emitter.cpp:818` 之后的调用基址调整。
3. `"hello"` 是字符串常量，`StringExpr` 的访问器在 `src/compiler/codegen/expression_emitter.cpp:329`，最终用 `LOADK` 放入参数寄存器。
4. `CALL` 由 `codeABC(OpCode::CALL, base, bArg, 2)` 生成，随后语句层把 C 改为 1，见 `src/compiler/codegen/expression_emitter.cpp:930`。
5. chunk 末尾补 `RETURN A=0 B=1`，见 `src/compiler/codegen/codegen.cpp:90`。
