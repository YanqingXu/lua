...
| 1.5 | Hello World 端到端 | `VM Dispatch` / `Value Pipeline` | `docs/walkthroughs/hello-world.md`, `src/compiler/codegen/expression_emitter.cpp`, `src/vm/vm.cpp`, `src/lib/baselib.cpp` | `print("hello")` 如何从源码走到字节码、VM 调度和 C 函数输出 |
...
| 想理解 | 先运行 | 再读 |
|---|---|---|
| 名字如何绑定到 local/global/upvalue | `bin\lua_test.exe --filter "Symbol Binding"` | `src/compiler/codegen/name_binder.cpp`, `src/compiler/codegen/codegen_binding.cpp` |
...
| 条件和短路如何避免错误物化 | `bin\lua_test.exe --filter "Codegen Conditions"` | `src/compiler/codegen/expression_emitter.cpp`, `src/compiler/codegen/jump_patcher.cpp`, `src/compiler/codegen/codegen_types.hpp` |
