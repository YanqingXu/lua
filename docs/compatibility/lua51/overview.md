...
- `lua51-differential` 使用官方 Lua 5.1 和本解释器运行同一批 Lua 探针，比较 stdout、stderr、退出码，并在 stdout 中编码返回类型、错误类别与 GC 弱引用副作用；`lua51-c-api-differential` 另将同一纯 C probe 分别链接两套 Runtime，覆盖核心表、遍历、比较、拼接、类型/线程/GC、auxlib，以及 stack/info/local/hook 调试入口。
