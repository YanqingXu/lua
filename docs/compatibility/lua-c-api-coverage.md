...
| 原生 C 模块 | `package.loadlib` / C searcher 使用 context-owned `NativeModuleRegistry` | 纯 C `lua.h` fixture 由 `lua_app`、公开 API embedding host 和双-context host 动态加载；open/init 错误、per-state 状态、lease cache、关闭与重载 | 官方模块源码不作为本项目二进制 ABI 替代证据 | Windows CMake 已验证 host import table；Linux/Windows CI 均运行 `api-contract` / `native-module` CTest |
...
结果为 39 个测试、931 个断言、0 failures。机器合同另包含 123 个官方公共函数：60 个 `PASS`、0 个 `XFAIL`、63 个显式 `UNSUPPORTED`。原始 `api.lua` 另以以下 exact TestC 门禁通过：
