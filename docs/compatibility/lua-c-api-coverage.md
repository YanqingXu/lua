...
函数兼容状态与项目实际导出面是两个独立集合：官方函数合同当前为 100 个 `PASS`，而项目公开头文件声明 107 个真实函数。合同检查器会从头文件自动枚举这 107 个函数，并要求 C 链接探针、C++ 精确签名断言、Windows `.def` 和 Linux version script 与之完全一致；项目额外入口 `lua_open`、global get/set、三个 auxlib helper 和安全线程扩展 `lua_trynewthread` 因而也必须可从共享库链接。相同消费者分别链接 `lua_core` 静态库和 `lua_public_api_shared` 动态库，避免静态链接掩盖导出缺口。
...
结果为 45 个测试、1144 个断言、0 failures。机器合同另包含 123 个官方公共函数：100 个 `PASS`、0 个 `XFAIL`、23 个显式 `UNSUPPORTED`。项目头文件的当前公开面另由 107 个真实函数、37 个宏、24 个枚举常量和 9 个 typedef 的穷尽式编译合同保护。完整 Debug/Release strict 套件为 740 个测试、4700 个断言、0 failures。原始 `api.lua` 另以以下 exact TestC 门禁通过：
...
`api-contract` 标签同时运行静态消费者、独立共享库消费者和候选 C API probe；Windows DLL 的导出面必须与 `.def` 中的 107 个符号完全一致，Linux shared object 则由版本脚本只公开同一集合。Linux Clang Debug 另外将 `lua51_c_api_differential_probe.c` 分别链接官方 Lua 5.1 和本项目，逐字节比较退出码、stdout 与 stderr，并上传 JSON 证据。
...
2. 继续补齐下一批 debug C API：stack/info/local/hook；不能用语言层 debug 库或内部 trace helper 替代公开 API 证据。
