...
- `tools/run_value_result_private_trial.ps1` 使用独立 CMake 构建目录打开 `LUA_VALUE_RESULT_PRIVATE_LEGACY_FIELDS=ON`，用于验证 `ValueResult` 旧字段 private mirror 试运行。
...
- 最近验证的测试计数：559 个 registered tests，2797 个 assertion results，0 failures。
...
- 本地质量门入口：`tools/run_quality_gate.ps1`，包括 clang-format、clang-tidy smoke、opcode coverage matrix、ValueResult legacy field fence、MSBuild、documentation drift 和 unit tests。
...
- `src/common/diagnostics.hpp` 提供 `LUA_SUPPRESS_DEPRECATED_DECLARATIONS_BEGIN/END`，用于少数受控兼容边界局部抑制 deprecation warning。
- `tools/check_value_result_legacy_fields.ps1` 已接入 `run_quality_gate.ps1`，用于阻止 `ValueResult` 旧字段直接访问回流。
- `tools/run_value_result_private_trial.ps1` 已验证 `ValueResult` 旧字段在宏开关下进入 private 区域时，CMake/CTest 仍可通过；`tools/test_quality_gate.ps1` 会检查该试运行入口和 CMake option 不漂移。
...
- `docs/compiler/codegen-responsibility-map.md` 记录了 PR-78 后的 CodeGenerator 职责地图；`JumpPatcher` 已抽出 jump-list / pending-jump / PC offset 回填边界，`ScopeManager` 已抽出 local / block / upvalue 作用域生命周期边界，`ExpressionEmitter` 已抽出 ValueResult / CondResult / CallResultInfo / LValueRef 表达式通道边界，`StatementEmitter` 已抽出 statement / block lowering 边界，`ValueResult` 已有兼容式 `std::variant` payload 原型，且 `ExpressionEmitter` 的核心读取辅助路径和 compiler 单测的普通断言读取面已迁移到 payload visitor；合法 legacy mirror 读取集中到 `legacyFields()` 快照桥，旧公开字段已标记 `[[deprecated]]`，内部同步和 `detail::ValueResultLegacyMirrorProbe` 使用显式 warning fence 保持 0 warning 门禁；PR-77 评估结论为暂不立即删除旧字段，并以质量门脚本阻止新增普通直接访问；PR-78 已证明 `LUA_VALUE_RESULT_PRIVATE_LEGACY_FIELDS` 打开后 private mirror 构建和 CTest 可通过；`Codegen Characterization`、`Jump Patcher`、`Scope Manager`、`Expression Emitter`、`Statement Emitter`、`Symbol Binding` 与 `Codegen Result Types` 测试套件共同锁住 statement lowering、jump patching、repeat-until scope、generic-for、表达式 lowering 和 ValueResult payload 同步行为。
