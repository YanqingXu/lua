...
- 最近一次完整绿跑测试计数：668 个 registered tests，3406 个 assertion results，0 failures。
...
- Lua 5.1 官方测试套件已放入 `tests/lua/official/`，并通过 `Lua 5.1 Official Suite` 单元测试以 staged smoke 形式运行 `all.lua`；当前官方子脚本 skip 表为 0，`_soft=true` 用于保持官方 semantic smoke 在单元测试中可控，当前 bounded smoke 覆盖到 `vararg.lua`。官方 suite 当前 11 个注册门禁在默认 512 MB cap 下通过。post-vararg tail 已拆成单脚本门禁：`closure.lua`、`errors.lua`、`math.lua`、`files.lua` 在 128 MB cap 下通过；closure 后 global cleanup tail 当前隔离为 known gap guard。`sort.lua` / `verybig.lua` 的完整 dump/undump 压力路径由 `tools/run_lua51_official_slow.ps1` 作为 opt-in slow gate 覆盖。
- `Codegen Characterization` 已新增 Lua 5.1 `T.listcode` parity 护栏，覆盖显式 nil local 的
  `LOADNIL` range merge、顶层未捕获且后续不可读局部变量的 nil dead-store elision、arithmetic
  constant folding、direct contiguous local return、`a = a` 自赋值消除、local/table assignment
  register reuse、concat chain merge 和常量 `not not` LOADBOOL 规约目标；动态 boolean/jump
  normalization 与 broader `T.listcode` fixture parity 仍是剩余缺口。
...
- `tools/check_value_result_variant_only.ps1` 已接入 `run_quality_gate.ps1`，用于阻止 `ValueResult` 兼容符号、private trial 宏和旧写入口回流。
- `tools/check_opcode_coverage_matrix.ps1` 从 `src/compiler/opcode.hpp` 解析 38 条 opcode，校验
  `tests/unit/vm/opcode_coverage_matrix.md` 的可读矩阵，并通过
  `tests/unit/vm/opcode_coverage_contract.json` 与 `bin/lua_test.exe --list` 验证 Positive、Boundary、
  Metamethod 覆盖引用的精确 `Suite::Test` ID 和源码路径。
- `tools/check_c_style_patterns.ps1` 使用 `tools/c_style_allowlist.json` 的
  `rule/path/line/textHash/rationale` 位置基线；严格产品例外不再使用数量门，tests/advisory 存量可通过
  显式 `-UpdateBaseline -TestScope All` 递减。
...
- `CodeGenerator` 的 public API 保留在 `src/compiler/codegen/codegen.hpp`，实现已物理拆分到 `src/compiler/codegen/codegen.cpp`、`src/compiler/codegen/codegen_binding.cpp`、`src/compiler/codegen/name_binder.cpp`、`src/compiler/codegen/codegen_ops.hpp`、`src/compiler/codegen/function_compiler.cpp`、`src/compiler/codegen/expression_emitter.cpp`、`src/compiler/codegen/statement_emitter.cpp`、`src/compiler/codegen/jump_patcher.cpp` 和 `src/compiler/codegen/codegen_stmt.cpp`。
- `docs/compiler/codegen-responsibility-map.md` 记录了当前 CodeGenerator 职责地图；`JumpPatcher` 已抽出 jump-list / pending-jump / PC offset 回填边界，`ScopeManager` 已抽出 local / block / upvalue 作用域生命周期边界，`ExpressionEmitter` 已抽出 ValueResult / CondResult / CallResultInfo / LValueRef 表达式通道边界，`StatementEmitter` 已抽出 statement / block lowering 边界，`NameBinder` 已抽出 Local -> Upvalue -> Global 名字解析与 `SymbolRef` 转换边界，`CodegenOps` 已抽出共享指令发射、参数回填、行号 guard 和寄存器 guard / frame helper，`FunctionCompiler` 已抽出子函数 `Proto` 编译、closure upvalue 和 debug metadata 边界；`ValueResult` 已完成 variant-only 迁移，旧字段 mirror、`legacyFields()`、drift probe、private trial 宏和 mutable payload 写入口均已删除；`Codegen Characterization`、`Jump Patcher`、`Scope Manager`、`Expression Emitter`、`Statement Emitter`、`Symbol Binding` 与 `Codegen Result Types` 测试套件共同锁住 statement lowering、jump patching、repeat-until scope、generic-for、表达式 lowering 和 ValueResult payload 行为。
