...
- `lua_app.vcxproj` / `lua_app`：当前脚本执行和 REPL 可执行目标；REPL 支持 `.help`、`.bytecode <expr|chunk>`、`.ast <expr|chunk>`、`.gc [stats|collect|strategy|help]`、Tab 补全、REPL 终端彩色错误、历史记录和 `=expr` 快速求值；实现已拆分为 `src/repl.cpp` 会话入口和 `src/repl/repl_*` 子模块。
...
- 最近验证的测试计数：530 个 registered tests，2641 个 assertion results，0 failures。
