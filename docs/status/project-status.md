...
- `lua_app.vcxproj` / `lua_app`：当前脚本执行和 REPL 可执行目标；REPL 支持 `.help`、`.bytecode <expr|chunk>`、`.ast <expr|chunk>`、`.gc [stats|collect|strategy|help]`、历史记录和 `=expr` 快速求值。
...
- 最近验证的测试计数：524 个 registered tests，2623 个 assertion results，0 failures。
