@echo off
REM Lua Interpreter Project Code Standards Check Script (ASCII Only)
REM Usage: check_standards_ascii.bat [file_path]
...
REM Check thread safety patterns
echo.
echo [INFO] Checking thread safety patterns...

findstr /C:"mutex" /C:"lock" /C:"atomic" "%target_file%" >nul 2>&1
if !errorlevel! equ 0 (
    echo [PASS] Found thread safety mechanisms
) else (
    echo [WARN] No explicit thread safety mechanisms found
)
...
if !check_failed! equ 0 (
    ...
    echo   - AddressSanitizer: Memory safety checks
    echo.
    echo To integrate into build system:
    echo   include^(tools/cmake_standards.cmake^)
    exit /b 0
) else (
    ...
    echo   1. Replace all primitive types with types.hpp definitions
    echo      - std::string    -^>  Str
    echo      - int           -^>  i32
    echo      - double        -^>  f64
    echo      - float         -^>  f32
    echo   2. Add #include "types.hpp" at the top of the file
    echo   3. Change all comments to English
    echo   4. Use smart pointers instead of raw pointers
    echo   5. Add Doxygen-style documentation for public interfaces
    echo   6. Consider thread safety for shared data
    echo.
    echo Example fixes:
    echo   Before: std::string name;
    echo   After:  Str name;
    echo.
    echo   Before: int count = 0;
    echo   After:  i32 count = 0;
    exit /b 1
)
