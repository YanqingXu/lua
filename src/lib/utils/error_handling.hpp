...
namespace Lua {
    namespace Lib {
        ...
        public:
            LibException(LibErrorCode code, StrView message) : LuaException(std::string(message)), errorCode(code) {}
            LibException(LibErrorCode code, const Str& message) : LuaException(message), errorCode(code) {}
            
            const char* what() const noexcept override {
                return LuaException::what();
            }
            
            LibErrorCode getErrorCode() const { return errorCode; }
         ...
         * 提供错误处理相关的函数
         */
        class ErrorHandlingLib : public Lib::LibModule {
        public:
        ...
        // 为了向后兼容，创建别名
        namespace ErrorUtilsCompat = ErrorUtils;
    } // namespace Lib
}