...
namespace Lua {
    ...
    LibManager::LibManager(std::shared_ptr<Lib::LibContext> context)
        : globalRegistry_(std::make_unique<Lib::LibFuncRegistry>())
        , context_(context ? context : std::make_shared<Lib::LibContext>()) {
    }

    void LibManager::registerModule(std::unique_ptr<Lib::LibModule> module, LoadStrategy strategy) {
        if (!module) {
        ...
        moduleRegistries_[name] = std::make_unique<Lib::LibFuncRegistry>();
    }
    ...
    const Lib::FunctionMetadata* LibManager::getFunctionMetadata(StrView name) const {
    ...
    Lib::LibFuncRegistry& LibManager::getRegistry() {
        return *globalRegistry_;
    }

    const Lib::LibFuncRegistry& LibManager::getRegistry() const {
        return *globalRegistry_;
    }

    Lib::LibContext& LibManager::getContext() {
        return *context_;
    }

    const Lib::LibContext& LibManager::getContext() const {
        return *context_;
    }
    ...
    bool LibManager::loadModuleInternal(const Str& name, State* state) {
        ...
        try {
            ...
            std::unique_ptr<Lib::LibModule> module;
            ...
            if (!registry) {
                registry = std::make_unique<Lib::LibFuncRegistry>();
            }
            ...
            if (moduleInfo_[name].status == ModuleStatus::Loaded) {
                ...
                for (const auto& funcName : functionNames) {
                    if (auto meta = registry->getFunctionMetadata(funcName)) {
                        ...
                        (void)meta; // Suppress unused variable warning
                    }
                }
            }
        }
...
namespace Lua::ManagerFactory {
    std::unique_ptr<LibManager> createStandardManager() {
        auto context = std::make_shared<Lib::LibContext>();
        context->setConfig("standard_mode", true);

        auto manager = std::make_unique<LibManager>(context);
        manager->registerModule(Lua::createBaseLib());
