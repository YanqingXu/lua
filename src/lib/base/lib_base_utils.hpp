...
public:
     ...
     */
    static std::optional<double> stringToNumber(std::string_view str, int base = 10);
...
public:
     ...
     */
    static void checkArgCount(State* state, int expected, const char* funcName);

    /**
     * Check argument count in range
     */
    static void checkArgCount(State* state, int minArgs, int maxArgs, const char* funcName);

    /**
     * Check if argument is a number and return it
     */
    static Value checkNumber(State* state, int index, const char* funcName);

    /**
     * Check if argument is a table and return it
     */
    static Value checkTable(State* state, int index, const char* funcName);

    /**
     * Check if argument is a string and return it
     */
    static Value checkString(State* state, int index, const char* funcName);
};
...
public:
     ...
     */
    static void error(State* state, const std::string& message, int level = 1);

    /**
     * Raise an argument error
     */
    static void argumentError(State* state, int argIndex, const std::string& message);

    /**
     * Raise a type error
     */
    static void typeError(State* state, int argIndex, const std::string& expectedType);
};
