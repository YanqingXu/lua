 ...
 * @brief 虚拟机执行器（内部实现类）
 * 
 * 封装字节码执行所需的全部状态和指令处理逻辑。
 * 作为局部对象在 VM::execute / VM::executeProto 中创建和销毁，
 * 与 Lua 5.1 C 实现中 luaV_execute() 使用局部变量的设计一致。
 */
class VMExecutor {
public:
    explicit VMExecutor(LuaState* L);
    ~VMExecutor() = default;
...
VMExecutor::VMExecutor(LuaState* L)
    : L_(L)
...
void VMExecutor::execute(Function* func) {
    if (!func) {
...
void VMExecutor::executeProto(Proto* proto, i32 nexeccalls) {
...
void VMExecutor::updateBasePointer() {
...
void VMExecutor::ensureStackSpace(usize needed) {
    Stack& stack = L_->getStack();
 ...
 */
void VMExecutor::arith(OpCode op, i32 a, i32 b, i32 c) {
    Value left = RK(b);
    Value right = RK(c);
...
void VMExecutor::compare(OpCode op, i32 a, i32 b, i32 c) {
    // 实现完整的比较操作，支持元方法
...
void VMExecutor::doJump(i32 offset) {
    pc_ += offset;
}
...
bool VMExecutor::precall(i32 funcIndex, i32 nArgs, i32 nResults) {
    Stack& stack = L_->getStack();
...
void VMExecutor::postcall(i32 funcPos, i32 wantedResults, usize firstResult) {
...
void VMExecutor::validateAndCheckDepth(Proto* proto, i32 nexeccalls) {
    if (!proto) {
...
void VMExecutor::initializeExecutionContext() {
...
inline void VMExecutor::executeMove(i32 a, i32 b) {
    // R(A) := R(B)
    R(a) = R(b);
}

inline void VMExecutor::executeLoadK(i32 a, i32 bx) {
    // R(A) := K(Bx)
    R(a) = K(bx);
}

inline void VMExecutor::executeLoadBool(i32 a, i32 b, i32 c) {
    // R(A) := (bool)B; if (C) pc++
    R(a) = Value(b != 0);
    if (c != 0) {
        pc_++;
...
inline void VMExecutor::executeLoadNil(i32 a, i32 b) {
    // R(A) := ... := R(B) := nil
    for (i32 i = a; i <= b; i++) {
        R(i) = Value();  // 默认构造为nil
    }
...
void VMExecutor::executeGetGlobal(i32 a, i32 bx) {
    // R(A) := Gbl[K(Bx)]
...
void VMExecutor::executeSetGlobal(i32 a, i32 bx) {
    // Gbl[K(Bx)] := R(A)
    // 使用当前函数的环境表（Lua 5.1兼容）
    const Value& key = K(bx);
...
void VMExecutor::executeGetTable(i32 a, i32 b, i32 c) {
    // R(A) := R(B)[RK(C)]
    // 支持__index元方法
...
void VMExecutor::executeSetTable(i32 a, i32 b, i32 c) {
    // R(A)[RK(B)] := RK(C)
    // 支持__newindex元方法
...
void VMExecutor::executeNewTable(i32 a) {
    // R(A) := {} (size = B,C)
    Table* table = new Table();
    R(a) = Value(table);
}

void VMExecutor::executeSelf(i32 a, i32 b, i32 c) {
...
void VMExecutor::executeSetList(i32 a, i32 b, i32 c) {
...
void VMExecutor::executeUnm(i32 a, i32 b) {
    // R(A) := -R(B)
    Value val = R(b);
...
void VMExecutor::executeNot(i32 a, i32 b) {
    // R(A) := not R(B)
    R(a) = Value(!R(b).isTrue());
}

void VMExecutor::executeLen(i32 a, i32 b) {
    // R(A) := length of R(B)
    // 支持__len元方法
    Value& val = R(b);
...
void VMExecutor::executeConcat(i32 a, i32 b, i32 c) {
    // R(A) := R(B).. ... ..R(C)
...
void VMExecutor::executeTest(i32 a, i32 c) {
...
void VMExecutor::executeTestSet(i32 a, i32 b, i32 c) {
...
void VMExecutor::executeGetUpval(i32 a, i32 b) {
    // R(A) := UpValue[B]
    if (!currentFunc_) {
...
void VMExecutor::executeSetUpval(i32 a, i32 b) {
    // UpValue[B] := R(A)
    if (!currentFunc_) {
...
void VMExecutor::executeClose(i32 a) {
...
void VMExecutor::executeForLoop(i32 a, i32 sbx) {
...
void VMExecutor::executeForPrep(i32 a, i32 sbx) {
    // R(A)-=R(A+2); pc+=sBx
    if (!R(a).isNumber() || !R(a + 1).isNumber() || !R(a + 2).isNumber()) {
...
void VMExecutor::executeTForLoop(i32 a, i32 c) {
...
void VMExecutor::executeClosure(i32 a, i32 bx) {
...
void VMExecutor::executeVararg(i32 a, i32 b) {
...
bool VMExecutor::executeCall(i32 a, i32 b, i32 c, i32& nexeccalls) {
...
bool VMExecutor::executeTailCall(i32 a, i32 b) {
...
bool VMExecutor::executeReturn(i32 a, i32 b, i32& nexeccalls) {
    // return R(A), ... ,R(A+B-2)
 ...
 */
bool VMExecutor::tryArithMetamethod(TMS op, const Value& left, const Value& right, Value& result) {
    // 调用二元运算元方法
 ...
 */
void VMExecutor::callMetamethod(const Value& metamethod, const Value& arg1,
                       const Value& arg2, Value& result) {
    // 调用元方法并获取返回值
...
void execute(LuaState* L, Function* func) {
    VMExecutor executor(L);
    executor.execute(func);
}

void executeProto(LuaState* L, Proto* proto, i32 nexeccalls) {
    VMExecutor executor(L);
    executor.executeProto(proto, nexeccalls);
}
