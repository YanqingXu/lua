...
[![Tests](https://img.shields.io/badge/tests-3077%2F3077-brightgreen)]()
...
- `bin/lua_test.exe`：621 个注册测试，3077 个结果，0 失败（0 failures）
- `tests/lua/official/all.lua`：Lua 5.1 官方测试套件已接入 staged smoke；当前通过 skip 表记录并跳过 7 个已知未兼容/重压脚本；`constructs.lua` 的 16k 次动态编译压力循环在单元测试入口中裁剪为小样本；`gc.lua` 已可完整 standalone 通过，但 staged smoke 仍跳过其 heavyweight GC/finalizer 压力段以控制单元测试耗时；`big.lua` 已按官方 `all.lua` 的 `coroutine.wrap(loadfile(...))` 路径纳入 staged smoke
...
> **兼容性审计记录（2026-05-29）**：已对 README、本地 `src/lib/` 标准库实现、`src/vm/` 指令执行逻辑、GC/元方法相关核心代码进行核对。`bin/lua_test.exe` 当前为 621 个注册测试、3077 个结果、0 失败；这说明项目内测试覆盖的路径稳定，但不等价于 Lua 5.1.5 官方语义已经达到 95% 兼容。Lua 5.1 官方测试套件已以 staged smoke 形式接入，当前 skip 表仍标记了 7 个官方子脚本，`literals.lua`、`calls.lua`、`attrib.lua`、`locals.lua`、`constructs.lua`、`vararg.lua`、`strings.lua`、`math.lua`、`nextvar.lua`、`errors.lua`、`sort.lua`、`pm.lua`、`closure.lua`、`events.lua` 与 `big.lua` 已从 skip 表移除；`gc.lua` 与 `db.lua` 可由 `bin/lua_app.exe` 完整单独执行通过，但 staged smoke 仍保留跳过对应重压/调试集成段。
...
当前测试入口会输出真实注册测试数和断言结果数；最近一次验证为 621 个注册测试、3077 个结果、0 失败。
