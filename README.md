...
- `src/api/lapi.cpp` 与 `src/api/lauxlib.cpp` 已形成 Lua 5.1 C API 嵌入 MVP：protected status API 不泄漏 C++ 异常，官方 123 个公共函数有机器可读三态合同，纯 C consumer 与独立 `.dll/.so` 模块通过公开头文件编译/链接/加载。当前 100 个函数为 PASS、23 个显式 UNSUPPORTED，项目公开面为 107 个真实函数；核心表、遍历、比较、拼接、类型转换、线程身份、GC 控制及完整 auxlib API 另由同一纯 C probe 对官方 Lua 5.1 做差分。allocator-backed hard limit 仍不宣称完成，边界见 [内存合同](docs/runtime/memory-contract.md)。
...
测试运行器会在输出中报告实时测试数量和断言结果。2026-07-15 的完整 Debug/Release strict 基线为 **740 registered tests, 4700 assertion results, 0 failures**；其中 `Lua C API` suite 为 45 个测试、1144 个断言、0 failures，原始 `api.lua with T module` 也完整运行到 `OK`。新增回归后需用文档漂移门禁同步这一基线。
