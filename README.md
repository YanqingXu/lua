...
[![Tests](https://img.shields.io/badge/tests-2684%2F2684-brightgreen)]()
...
- `bin/lua_test.exe`：539 个注册测试，2684 个结果，0 失败（0 failures）
...
> **兼容性审计记录（2026-05-23）**：已对 README、本地 `src/lib/` 标准库实现、`src/vm/` 指令执行逻辑、GC/元方法相关核心代码进行核对。`bin/lua_test.exe` 当前为 539 个注册测试、2684 个结果、0 失败；这说明项目内测试覆盖的路径稳定，但不等价于 Lua 5.1.5 官方语义已经达到 95% 兼容。
...
当前测试入口会输出真实注册测试数和断言结果数；最近一次验证为 539 个注册测试、2684 个结果、0 失败。
