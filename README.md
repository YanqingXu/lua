...
[![Tests](https://img.shields.io/badge/tests-2919%2F2919-brightgreen)]()
...
- `bin/lua_test.exe`：594 个注册测试，2919 个结果，0 失败（0 failures）
- `tests/lua/official/all.lua`：Lua 5.1 官方测试套件已接入 staged smoke；当前通过 skip 表记录并跳过 14 个已知未兼容脚本；`constructs.lua` 的 16k 次动态编译压力循环在单元测试入口中裁剪为小样本，完整脚本仍可通过 `lua_app.exe` 单独执行
...
当前测试入口会输出真实注册测试数和断言结果数；最近一次验证为 594 个注册测试、2919 个结果、0 失败。
