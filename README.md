...
测试运行器会在输出中报告实时测试数量和断言结果。2026-07-16 的完整 Debug/Release strict 基线为 **755 registered tests, 4946 assertion results, 0 failures**；其中 `Lua C API` suite 为 50 个测试、1275 个断言、0 failures，原始 `api.lua with T module` 也完整运行到 `OK`。新增回归后需用文档漂移门禁同步这一基线。
